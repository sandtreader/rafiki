...
import { TextField, Stack, Chip, Autocomplete } from '@mui/material';
...
export default function ChipArrayField<T>({
  ...
  const unusedItems =
    allItems?.filter((ai) => !items.find((i) => ai.id === i.id)) || [];
          ...
          items.map((item) => {
            ...
            return (
              <Chip
                ...
                onDelete={editable ? () => deleteItem(field, item) : undefined}
              />
            );
          })
        }
...

      {
        ...
        editable && unusedItems.length > 0 && (
          <Autocomplete
            blurOnSelect={true}
            getOptionLabel={(item) =>
              (field.getItemName && field.getItemName(item)) || String(item.id)
            }
            options={unusedItems}
            renderInput={(params) => (
              <TextField {...params} fullWidth label="Add" />
            )}
            value={null}
            onChange={(e, value) => {
